...
void Bloodworks::initImplementation()
{
	...
	loadAutoGenerated = config->getModsAreDisabled();
	...
	mapSize = Vec2(DEFAULT_MAP_SIZE);
	mapRect.set(-mapSize * 0.5f, +mapSize * 0.5f);
	maxSetMapSize = mapSize + 2.0f;
...
void Bloodworks::loadMod(const std::string& path, bool loadOnlyModData)
{
	...
	for (auto& f : files)
	{
		if ( f.file == "helpers.lua")
		{
			loadLuaFile(f.folder + f.file);
		}
	}
	for (auto& f : files)
	{
...
bool Bloodworks::isInvalidDefault() const
{
	return invalidDefault;
}

void Bloodworks::setMapSize(const Vec2& size)
{
	if ((mapSize - size).lengthSquared() < 1.0f)
	{
		return;
	}
	...
	if (maxSetMapSize.x < mapSize.x || maxSetMapSize.y < mapSize.y)
	{
		delete bloodRenderable;
		bloodRenderable = new BloodRenderable(this);
		bloodRenderable->init();
		addRenderable(bloodRenderable, BACKGROUND + 1);
		...
		bulletController->updateGrid();
		monsterController->updateGrid();
		collisionController->updateGrid();
...
ExplosionData& Bloodworks::addExplosion(const Vec2& pos, float maxScale, float scaleSpeed, int minDamage, int maxDamage, float startTime, bool damagePlayer, sol::function onHit, bool noParticle )
{
	return explosionController->addExplosion(pos, maxScale, scaleSpeed, minDamage, maxDamage, startTime, damagePlayer, onHit, noParticle);
}
