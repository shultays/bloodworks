...
void Bloodworks::initImplementation()
{
	...
	loadLuaFile("resources/helpers.lua");
	loadLuaFile("resources/missions/helpers.lua");
	loadLuaFile("resources/guns/helpers.lua");
	loadLuaFile("resources/monsters/helpers.lua");
...
{
	autoGenerating = true;
	autoGeneratedFile.open("example.txt");

	autoGeneratedFile << "#include <unordered_map>" "\n";

	autoGeneratedFile << "std::unordered_map<std::string, std::string> defaultFiles = {" "\n";
}
...
void Bloodworks::parseJson(nlohmann::json& j, DirentHelper::File& f, bool loadOnlyModData)
{
	...
	{
		cParticleTemplate *particleTemplate = new cParticleTemplate(this, j, f);
	...
	{
		GameObjectTemplate *gameObjectTemplate = new GameObjectTemplate(this, j, f);
