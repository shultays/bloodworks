...
void Bloodworks::stopAutoGenerating()
{
	if (autoGenerating)
	{
		...
		autoGeneratedFile << "std::vector<std::string> autoLoadHelpers = {" "\n";

		for (auto& t : loadedLuaFiles)
		{
			if (endsWith(t, "helpers.lua"))
			{
				autoGeneratedFile << "\"" << t << "\"," "\n";
			}
		}
...
{
	if (loadedLuaFiles.findIndex(scriptFilePath) >= 0)
	{
		return;
	}
	loadedLuaFiles.push_back(scriptFilePath);
