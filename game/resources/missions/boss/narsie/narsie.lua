...
function NarSie.init(monster, min)
    ...
    data.attacks = { NarSie.ringBullets, NarSie.arcBullets, NarSie.fireballs, NarSie.spawnSpiders }
...
function NarSie.onTick(monster)
    ...
    if data.currentAttack == nil then
        ...
        if data.defaultShootTimer < 0.0 then
...
function NarSie.spawnSpiders(monster, init)
    if init then
        ...
        data.interval = lerp(0.15, 0.5, clamp(min * 0.1))
        data.min = min
